...
__export(src_exports, {
  CapturePurchaseRequestPurchaseTypeEnum: () => CapturePurchaseRequestPurchaseTypeEnum,
  ...
  GetSubscription200ResponseSubscriptionsInnerTypeEnum: () => GetSubscription200ResponseSubscriptionsInnerTypeEnum,
  KeyManager: () => KeyManager,
  KeyManagerAbi: () => KeyManagerAbi,
  KeyOrderBy: () => Key_OrderBy,
  LockOrderBy: () => Lock_OrderBy,
  LocksmithService: () => LocksmithService,
  LocksmithServiceConfiguration: () => Configuration,
  OrderDirection: () => OrderDirection,
  SubgraphService: () => SubgraphService,
  TransferTypes: () => TransferTypes,
  TransferWithAuthorizationTypes: () => TransferWithAuthorizationTypes,
  WalletService: () => WalletService,
  Web3Service: () => Web3Service,
  approveTransfer: () => approveTransfer,
  getAllowance: () => getAllowance,
  getCurrentProvider: () => getCurrentProvider,
  getErc20BalanceForAddress: () => getErc20BalanceForAddress,
  getErc20Decimals: () => getErc20Decimals,
  getErc20TokenSymbol: () => getErc20TokenSymbol,
  getWeb3Provider: () => getWeb3Provider,
  recoverTransferAuthorization: () => recoverTransferAuthorization,
...
var utils_default = {
  toWei: (value, units) => import_ethers.ethers.utils.parseUnits(value, units),
  // This converts a string representation from a value to a number of units, based on the number of decimals passed in
  toDecimal: (value, decimals) => import_ethers.ethers.utils.parseUnits(value, decimals),
  hexlify: import_ethers.ethers.utils.hexlify,
  hexStripZeros: import_ethers.ethers.utils.hexStripZeros,
  bigNumberify: import_ethers.ethers.BigNumber.from,
  hexToNumberString: (num) => import_ethers.ethers.utils.formatUnits(import_ethers.ethers.BigNumber.from(num), "wei").replace(".0", ""),
  toChecksumAddress: import_ethers.ethers.utils.getAddress,
  fromWei: (num, units) => {
    return import_ethers.ethers.utils.formatUnits(import_ethers.ethers.BigNumber.from(num), units).replace(/\.0$/, "");
  },
  // This converts a string representation from a unit value to a higher base
  fromDecimal: (num, decimals) => {
    return import_ethers.ethers.utils.formatUnits(import_ethers.ethers.BigNumber.from(num), decimals).replace(/\.0$/, "");
  },
  isInfiniteKeys: (value) => {
    return import_ethers.ethers.BigNumber.from(value).eq(import_ethers.ethers.constants.MaxUint256);
  },
  isInfiniteDuration: (value) => {
    return import_ethers.ethers.BigNumber.from(value).eq(import_ethers.ethers.constants.MaxUint256);
  },
  toNumber: (value) => {
    return import_ethers.ethers.BigNumber.from(value).toNumber();
  },
  toRpcResultNumber: (value) => {
    const num = import_ethers.ethers.utils.hexlify(import_ethers.ethers.BigNumber.from(value));
    return import_ethers.ethers.utils.hexZeroPad(num, 32);
  },
  toRpcResultString: (str) => {
    return str;
  },
  utf8ToHex: (str) => import_ethers.ethers.utils.hexlify(str.length ? import_ethers.ethers.utils.toUtf8Bytes(str) : 0),
  sha3: import_ethers.ethers.utils.keccak256,
  verifyMessage: import_ethers.ethers.utils.verifyMessage,
  currencyAmountToBigNumber: (amount) => {
    const { decimals } = amount.currency;
    const fixed = import_ethers.ethers.FixedNumber.from(amount.toExact());
    const tokenScale = import_ethers.ethers.FixedNumber.from(
      import_ethers.ethers.BigNumber.from(10).pow(decimals)
    );
    return import_ethers.ethers.BigNumber.from(
...
var ETHERS_MAX_UINT = import_ethers2.ethers.constants.MaxUint256;
var UNLIMITED_KEYS_COUNT = -1;
var ZERO = import_ethers2.ethers.constants.AddressZero;
...
async function getErc20BalanceForAddress(erc20ContractAddress, address, provider) {
  const contract = new import_ethers3.ethers.Contract(erc20ContractAddress, erc20abi_default, provider);
...
async function getAllowance(erc20ContractAddress, lockContractAddress, provider, spenderAddress) {
  const contract = new import_ethers3.ethers.Contract(erc20ContractAddress, erc20abi_default, provider);
  let amount = import_ethers3.ethers.BigNumber.from(0);
  try {
...
async function approveTransfer(erc20ContractAddress, lockContractAddress, value, provider, signer) {
  const contract = new import_ethers3.ethers.Contract(erc20ContractAddress, erc20abi_default, signer);
...
var getDomain = async (chainId, erc20ContractAddress, provider) => {
  ...
  if (chainId === 137) {
    return {
      name: name2,
      version,
      salt: import_ethers3.ethers.utils.zeroPad(import_ethers3.ethers.utils.arrayify(137), 32),
      verifyingContract: import_ethers3.ethers.utils.getAddress(erc20ContractAddress)
    };
  }
  return {
    name: name2,
...
async function recoverTransferAuthorization(erc20ContractAddress, message, chainId, signature, provider) {
  const domain = await getDomain(chainId, erc20ContractAddress, provider);
  return import_ethers3.ethers.utils.verifyTypedData(
    domain,
...
async function transferWithAuthorization(erc20ContractAddress, message, signature, signer) {
  const contract = new import_ethers3.ethers.Contract(erc20ContractAddress, erc20abi_default, signer);
  const { v, r, s } = import_ethers3.ethers.utils.splitSignature(signature);
...
var import_types = __toESM2(require_dist());
var mainnet = {
  blockScan: {
...
var import_types2 = __toESM2(require_dist());
var gnosis = {
  blockScan: {
    url: (address) => `https://blockscan.com/address/${address}`
  },
...
var import_types3 = __toESM2(require_dist());
var polygon = {
  blockScan: {
    url: (address) => `https://blockscan.com/address/${address}`
  },
...
var import_types4 = __toESM2(require_dist());
var bsc = {
  blockScan: {
    url: (address) => `https://blockscan.com/address/${address}`
  },
...
var import_types5 = __toESM2(require_dist());
var optimism = {
  blockScan: {
    url: (address) => `https://blockscan.com/address/${address}`
  },
...
var import_types6 = __toESM2(require_dist());
var mumbai = {
  blockScan: {
    url: (address) => `https://blockscan.com/address/${address}`
  },
...
var import_types7 = __toESM2(require_dist());
var goerli = {
  blockScan: {
    url: (address) => `https://blockscan.com/address/${address}`
  },
...
var import_types8 = __toESM2(require_dist());
var celo = {
  blockScan: {
    url: (address) => `https://blockscan.com/address/${address}`
  },
...
var import_types9 = __toESM2(require_dist());
var arbitrum = {
  blockScan: {
    url: (address) => `https://blockscan.com/address/${address}`
  },
...
var import_types10 = __toESM2(require_dist());
var avalanche = {
  blockScan: {
    url: (address) => `https://blockscan.com/address/${address}`
  },
...
var import_types11 = __toESM2(require_dist());
var sepolia = {
  blockScan: {
    url: (address) => `https://blockscan.com/address/${address}`
  },
...
async function setMaxNumberOfKeys2({ lockAddress, gasRefundValue }, transactionOptions = {}, callback) {
  ...
  if (erc20Address !== import_ethers4.ethers.constants.AddressZero) {
    decimals = await getErc20Decimals(erc20Address, this.provider);
  }
  const refundValue = import_ethers4.ethers.utils.parseUnits(gasRefundValue, decimals);
...
async function addKeyGranter_default2({ lockAddress, keyGranter }, transactionOptions = {}, callback) {
  const lockContract = await this.getLockContract(lockAddress);
  const keyGranterRole = import_ethers5.ethers.utils.keccak256(
    import_ethers5.ethers.utils.toUtf8Bytes("KEY_GRANTER")
  );
...
async function isKeyGranter_default2(lockAddress, address, provider) {
  const lockContract = await this.getLockContract(lockAddress, provider);
  const keyGranterRole = import_ethers6.ethers.utils.keccak256(
    import_ethers6.ethers.utils.toUtf8Bytes("KEY_GRANTER")
  );
...
async function createLock_default6(lock, transactionOptions = {}, callback) {
  ...
  const lockInterface = new import_ethers7.ethers.utils.Interface(lockAbi);
...
var UnlockService = class {
  ...
  providerForNetwork(networkId) {
    ...
    return new import_ethers8.ethers.providers.JsonRpcBatchProvider(
  ...
  async _getUnlockVersionFromContract(address, provider) {
    ...
    try {
      ...
      const hash = import_ethers8.ethers.utils.sha256(opCode);
  ...
  getContract(address, contract, provider) {
    return new import_ethers8.ethers.Contract(address, contract.abi, provider);
  }
  ...
  async getHookContract({
    ...
    return new import_ethers8.ethers.Contract(address, abi, provider);
  }
};
...
var import_smart_order_router = require("@uniswap/smart-order-router");
var Web3Service = class extends UnlockService {
  ...
  async getLock(address, network, options = {
    ...
    lock.unlockContractAddress = import_ethers9.ethers.utils.getAddress(
      lock.unlockContractAddress
    );
    const previousDeployAddresses = (networkConfig.previousDeploys || []).map(
      (d) => import_ethers9.ethers.utils.getAddress(d.unlockAddress)
    );
    const isPreviousUnlockContract = previousDeployAddresses.includes(
      lock.unlockContractAddress
    );
    const isUnlockContract = import_ethers9.ethers.utils.getAddress(networkConfig.unlockAddress) === lock.unlockContractAddress;
  ...
  async numberOfOwners(lockAddress, network) {
    ...
    return import_ethers9.ethers.BigNumber.from(await lockContract.numberOfOwners()).toNumber();
  }
  /**
  ...
  async transferFeeBasisPoints(lockAddress, network) {
    ...
    return import_ethers9.ethers.BigNumber.from(
  ...
  async freeTrialLength({
    ...
    return import_ethers9.ethers.BigNumber.from(freeTrialLength).toNumber();
  }
  /**
  ...
  async refundPenaltyBasisPoints({
    ...
    return import_ethers9.ethers.BigNumber.from(refundPenaltyBasisPoints).toNumber();
  }
  /**
  ...
  async referrerFees({
    ...
    return import_ethers9.ethers.BigNumber.from(referrerFees).toNumber();
  }
  ...
  async resolveName(addressOrEns) {
    ...
    try {
      ...
      const isNotENS = import_ethers9.ethers.utils.isAddress(address);
      if (isNotENS) {
        return {
          input: address,
          address,
  ...
  async getUniswapRoute({
    ...
    const provider = this.providerForNetwork(network);
    const networkConfig = this.networks[network];
    const router = new import_smart_order_router.AlphaRouter({
      chainId: network,
      provider
    });
    const outputAmount = import_sdk_core.CurrencyAmount.fromRawAmount(tokenOut, amountOut);
    const routeArgs = [
      outputAmount,
      tokenIn,
      import_sdk_core.TradeType.EXACT_OUTPUT,
      {
        type: import_smart_order_router.SwapType.UNIVERSAL_ROUTER,
        recipient,
        slippageTolerance: new import_sdk_core.Percent(15, 100),
  ...
  async getGasRefundValue({
    ...
    let decimals = this.networks[network].nativeCurrency.decimals;
    const erc20Address = await lockContract.tokenAddress();
    if (erc20Address !== import_ethers9.ethers.constants.AddressZero) {
      decimals = await getErc20Decimals(erc20Address, provider);
    }
    return import_ethers9.ethers.utils.formatUnits(gasRefund, decimals);
  }
  /**
   * Get signer for `Password hook contract`
   */
  async getDiscountHookValues(params) {
    ...
    return import_ethers9.ethers.BigNumber.from(discountForSigner).toNumber();
  }
};
...
var CardPurchaser = class {
  ...
  providerForNetwork(network) {
    ...
    return new import_ethers10.ethers.providers.JsonRpcBatchProvider(
  ...
  getContract({ network, signer }) {
    ...
    const provider = this.providerForNetwork(network);
    const cardPurchaserContract = new import_ethers10.ethers.Contract(
...
var WalletService = class extends UnlockService {
  ...
  getUnlockSwapPurchaserContract({
    ...
    const provider = this.providerForNetwork(network);
    const swapPurchaserContract = new import_ethers11.ethers.Contract(
  ...
  async getAndSignAuthorizationsForTransferAndPurchase({
    ...
    const value = import_ethers11.ethers.utils.parseUnits(amount, 4).toHexString();
    const now = Math.floor((/* @__PURE__ */ new Date()).getTime() / 1e3);
    const transferMessage = {
      from: await this.signer.getAddress(),
      to: import_ethers11.ethers.utils.getAddress(cardPurchaserAddress),
      value,
      validAfter: 0,
      validBefore: now + 60 * 60 * 24,
      // Valid for 1 day (TODO: how do we handle funds when they are stuck?)
      nonce: import_ethers11.ethers.utils.hexlify(import_ethers11.ethers.utils.randomBytes(32))
      // 32 byte hex string
    };
...
var LocksmithService = class extends DefaultApi {
  ...
  static createSiweMessage(options) {
    return new import_siwe.SiweMessage(options);
  }
};
...
var SubgraphService = class {
  ...
  createSdk(networkId = 1) {
    ...
    const client = new import_graphql_request.GraphQLClient(network.subgraph.endpointV2);
...
var KeyManager = class {
  ...
  providerForNetwork(network) {
    ...
    return new import_ethers12.ethers.providers.JsonRpcBatchProvider(
      this.networks[network].provider,
      network
    );
  }
  ...
  getContract({ network, signer }) {
    ...
    const provider = this.providerForNetwork(network);
    const KeyManagerContract = new import_ethers12.ethers.Contract(
  ...
  createTransferAddress({ params }) {
    ...
    return import_ethers12.ethers.utils.id(JSON.stringify(item)).slice(0, 42);
  }
  /**
  ...
  getSignerForTransferSignature({
    ...
    const domain = this.getDomain(network);
    const recoveredAddress = import_ethers12.ethers.utils.verifyTypedData(
      domain,
      TransferTypes,
